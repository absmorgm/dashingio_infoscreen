...
# Config
appPageUrl = '/de/app/zenjob-die-app-für-nebenjobs/id1141020680'
...
SCHEDULER.every '30m', :first_in => 0 do |job|
  ...
  if response.code != "200"
    ...
    average_rating = response.body.scan( /(Version(en)?:(.)*?aria-label=[\"\'](?<num>.*?)Stern)/m )
...
=end
      ...
      if ( average_rating[0] ) # All Versions
        i = 0
        raw_string = average_rating[i][0].gsub('star', '')
